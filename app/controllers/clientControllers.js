...
const indexUrl = async (req, res) => {
	try {
		...
		$("*").each((_, element) => {
			...
			$(element)
				...
				.forEach((word) => {
					...
					wordWeights[lowerCased] =
						(wordWeights[lowerCased] || 0) + tagWeights[tagName];
				});
		});
		...
		words.forEach(async (word) => {
			try {
				...
				if (doc) {
					...
					if (!doc.weights.has(weight)) doc.weights[weight] = [url];
					else doc.weights[weight].push(url);

					await doc.save();
				} else {
					await KeywordsModel({
						word,
						weights: new Map([[weight, [url]]]),
					}).save();
				}
			} catch (error) {
				console.error(error);
		...
		await UrlsModel.findOneAndUpdate(
			{ url },
			{
				url,
				links,
		...
		if (statusCode) return res.status(statusCode).send({ error: message });
